...
export default function Home() {
  ...
  const getMembershipText = (user) => {
    ...
    const membershipLabel = user?.membership_label;
    const normalizedLabel = typeof membershipLabel === 'string' ? membershipLabel.trim() : membershipLabel;

    if (normalizedLabel === 1 || normalizedLabel === '1') {
      return 'Scratcher';
    }

    if (normalizedLabel === 0 || normalizedLabel === '0') {
      return 'New Scratcher';
    }

    if (typeof normalizedLabel === 'string' && normalizedLabel) {
      return normalizedLabel;
    }

    const avatarBadge = user?.membership_avatar_badge;
    if (avatarBadge === 1 || avatarBadge === '1') {
      return 'Scratcher';
    }

    if (avatarBadge === 0 || avatarBadge === '0') {
      return 'New Scratcher';
    }
